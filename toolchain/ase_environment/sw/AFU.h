...
class AFU {
  ...
  public:
    ...
    template <class T>
    T* malloc(size_t elements, PageOptions page_option = DEFAULT_PAGE_OPTION,
    ...
    template <class T>
    T* mallocNonvolatile(size_t elements,
                         PageOptions page_option = DEFAULT_PAGE_OPTION,
                         bool read_only = false) {

        opae::fpga::types::shared_buffer::ptr_t buf_handle;
        buf_handle = alloc(elements * sizeof(T), page_option, read_only);

        // Allow for a dangerous const_cast that eliminates the volatility of
        // the allocated data. This could potentially cause errors since the
        // compiler may perform optimizations without the knowledge of the FPGA.
        // However, removing the volatility allows the returned pointer to be
        // passed to functions that do not have volatile parameters (e.g.,
        // libraries).
  ...
  protected:
    ...
    alloc(size_t bytes, PageOptions page_option, bool read_only);
};
