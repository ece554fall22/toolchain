...
AFU::AFU(handle::ptr_t fpga_handle) : fpga_(fpga_handle) {
    ...
    if (fpga_handle == nullptr)
        throw runtime_error(
            "ERROR: AFU can't be constructed with a null handle.");
...
handle::ptr_t AFU::requestAfu(const char* uuid) {
    ...
    for (token::ptr_t a : accelerators) {
        try {
            ...
            // open() throws a busy exception when the requested accelerator is
            // in use. In this case, we want to do nothing, so just catch the
            // exception and try the next accelerator.
        }
    }
...
void AFU::reset() { fpga_->reset(); }

void AFU::write(uint64_t addr, uint64_t data) const {

    // This AFU wrapper class only supports 64-bit MMIO transfers, which
    // requires the 32-bit word address to be even.
    if (addr % 2 == 1) {
        throw runtime_error("ERROR: AFU::write requires even addresses due to "
                            "64-bit MMIO transfers");
    }
...
uint64_t AFU::read(uint64_t addr) const {
    ...
    // requires the 32-bit word address to be even.
    if (addr % 2 == 1) {
        throw runtime_error("ERROR AFU::read requires even addresses due to "
                            "64-bit MMIO transfers");
    }
...
void AFU::free(volatile void* ptr) {
    ...
    if (it == buffer_map_.end()) {
        throw std::runtime_error(
            "ERROR: AFU::free() called with pointer without shared buffer.");
    }

    buffer_map_.erase(it);
};

opae::fpga::types::shared_buffer::ptr_t
AFU::alloc(size_t bytes, PageOptions page_option, bool read_only) {
    ...
    buf_handle = opae::fpga::bbb::mpf::types::mpf_shared_buffer::allocate(
        mpf_, oage_aligned_bytes, read_only);
#else
    buf_handle = opae::fpga::bbb::mpf::types::mpf_shared_buffer::allocate(
        mpf_, page_aligned_bytes);
#endif
