...
Token Lexer::next() noexcept {
	while (true) {
		switch (peek()) {
			...
			case ':': return tokAtom(Token::Kind::COLON);
			case ',': return tokAtom(Token::Kind::COMMA);
			case '[': return tokAtom(Token::Kind::L_SQUARE);
			case '{': return tokAtom(Token::Kind::L_BRACE);
			case ']': return tokAtom(Token::Kind::R_SQUARE);
			case '}': return tokAtom(Token::Kind::R_BRACE);
			case '#': return tokAtom(Token::Kind::HASH);
			case '+':
				if (peek(+1) == '=') {
					// `+=`
