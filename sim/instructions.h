...
void addi(CPUState& cpu, MemSystem& mem, reg_idx rD, reg_idx rS, s<12> imm) {
    cpu.r[rD] = cpu.r[rS].asSigned() + imm;
}
...
void vmul(CPUState& cpu, MemSystem& mem, vreg_idx vD, vreg_idx vA, vreg_idx vB, u<4> mask) {
...
void flushicache(CPUState& cpu, MemSystem& mem) {
    mem.flushICache();
}

void flushdirty(CPUState& cpu, MemSystem& mem) {
    mem.flushDCacheDirty();
}

void flushclean(CPUState& cpu, MemSystem& mem) {
    mem.flushDCacheClean();
}
